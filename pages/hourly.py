...
def render_hourly_page():
    ...
    with html.Div(classes="hourly-page"):
        html.H1(
            "{{ location.name }} Weather",
            classes="text-h4 font-weight-bold mb-1 white--text"
        )
        html.Div(
            "As of {{ current_time_display }}",
            classes="text-body-2 grey--text mb-4"
        )
        
        # Hourly forecast card
        with v3.VCard(classes="hourly-card", elevation=0):
            with v3.VCardText(classes="pa-0"):
                # Use v-for with a template to group by date
                with html.Template(v_for="(group, groupIndex) in hourly_grouped", key="groupIndex"):
                    # Date header
                    html.Div(
                        "{{ group.date }}",
                        classes="date-header text-h6 font-weight-bold pa-4 pb-2"
                    )
                    ...
                    with v3.VExpansionPanels(variant="accordion", classes="hourly-panels"):
                        with v3.VExpansionPanel(
                            ...
                            with v3.VExpansionPanelTitle(classes="hourly-panel-header py-2"):
                                with v3.VRow(align="center", classes="w-100", no_gutters=True):
                                    ...
                                    with v3.VCol(cols=2, sm=1):
                                        with html.Div(classes="d-flex align-center"):
                                            v3.VIcon("mdi-water-outline", size="16", classes="mr-1", color="blue-lighten-3")
                                    ...
                                    with v3.VCol(cols=6, sm=4, md=2):
                                        with html.Div(classes="hourly-detail"):
                                            with html.Div(classes="d-flex align-center mb-1"):
                                                v3.VIcon("mdi-thermometer", size="16", classes="mr-1 opacity-60")
                                                html.Span("Feels Like", classes="text-caption opacity-60")
                                            html.Div("{{ hour.feels_like }}°", classes="text-body-2 font-weight-medium")
                                    
                                    with v3.VCol(cols=6, sm=4, md=2):
                                        with html.Div(classes="hourly-detail"):
                                            with html.Div(classes="d-flex align-center mb-1"):
                                                v3.VIcon("mdi-weather-windy", size="16", classes="mr-1 opacity-60")
                                                html.Span("Wind", classes="text-caption opacity-60")
                                            html.Div("{{ hour.wind_direction }} {{ hour.wind_speed }} mph", classes="text-body-2 font-weight-medium")
                                    
                                    with v3.VCol(cols=6, sm=4, md=2):
                                        with html.Div(classes="hourly-detail"):
                                            with html.Div(classes="d-flex align-center mb-1"):
                                                v3.VIcon("mdi-water-percent", size="16", classes="mr-1 opacity-60")
                                                html.Span("Humidity", classes="text-caption opacity-60")
                                            html.Div("{{ hour.humidity }}%", classes="text-body-2 font-weight-medium")
                                    
                                    with v3.VCol(cols=6, sm=4, md=2):
                                        with html.Div(classes="hourly-detail"):
                                            with html.Div(classes="d-flex align-center mb-1"):
                                                v3.VIcon("mdi-white-balance-sunny", size="16", classes="mr-1 opacity-60")
                                                html.Span("UV Index", classes="text-caption opacity-60")
                                            html.Div("{{ hour.uv_index }} of 11", classes="text-body-2 font-weight-medium")
                                    
                                    with v3.VCol(cols=6, sm=4, md=2):
                                        with html.Div(classes="hourly-detail"):
                                            with html.Div(classes="d-flex align-center mb-1"):
                                                v3.VIcon("mdi-cloud-outline", size="16", classes="mr-1 opacity-60")
                                                html.Span("Cloud Cover", classes="text-caption opacity-60")
                                            html.Div("{{ hour.cloud_cover }}%", classes="text-body-2 font-weight-medium")
                                    
                                    with v3.VCol(cols=6, sm=4, md=2):
                                        with html.Div(classes="hourly-detail"):
                                            with html.Div(classes="d-flex align-center mb-1"):
                                                v3.VIcon("mdi-umbrella-outline", size="16", classes="mr-1 opacity-60")
                                                html.Span("Precip Amount", classes="text-caption opacity-60")
